 ...
 */
export async function updateUserProfile(uid: string, data: Partial<Omit<UserProfile, 'createdAt' | 'updatedAt'>>): Promise<void> {
  const docRef = doc(db, 'users', uid); 
 ...
 */
export async function updateEmailVerificationStatus(uid: string, isVerified: boolean): Promise<void> {
  const docRef = doc(db, 'users', uid);
  await updateDoc(docRef, {
