...
import { collection, addDoc, getDocs, doc, updateDoc, Timestamp, query, where, getDoc } from 'firebase/firestore';
 ...
 */
export async function createCinemaRoom(roomData: Omit<CinemaRoom, 'id'>): Promise<string> {
  try {
 ...
 */
export async function updateCinemaRoom(roomId: string, data: Partial<Omit<CinemaRoom, 'id'>>): Promise<void> {
  try {
 ...
 */
export async function assignMovieToRoom(roomId: string, movieId: number, showtime: string): Promise<void> {
  try {
    const roomRef = doc(db, 'cinema_rooms', roomId);
    await updateDoc(roomRef, {
      currentMovie: {
...
export async function createBooking(bookingData: Omit<Booking, 'id' | 'createdAt'>): Promise<string> {
  try {
    ...
    const updatedSeats = room.seats.map(seat => ({
      ...seat,
      isAvailable: !bookingData.seats.some(bookedSeat => bookedSeat.seatId === seat.id),
    }));
